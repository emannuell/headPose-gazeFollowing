...
def deepMain():
    ...
    cap = cv2.VideoCapture('/home/emannuell/Documentos/mestrado/video01.webm')
    ...
    out = cv2.VideoWriter('my-gazefollowing04.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (1280, 480))
        ...
        if ret == True:
            ...
            if len(faces) > 0:
                for detected in faces:
                    ...
                    faceImage = img[detected[1]:detected[1] + detected[2], detected[0]:detected[3] + detected[0]]
                    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                    eye_center = detect_eyes(gray)
                    if len(eye_center) > 0:
                        x_ = face.x
                        y_ = face.y
                        w_ = face.w
                        ...
                        faceBbox = face.x, face.y, face.x + face.w, face.y + face.h
                        ...
                        img = cv2.circle(img, (eye_center[0], eye_center[1]), 2, (255, 0, 255), thickness=2) # Magento
                        heatmap, p_x, p_y = test(net, originalImg, (center_x, center_y), headPoseAngles)
